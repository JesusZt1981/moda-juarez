...
function itemRow(i){const c=calc(i);return `<tr data-id="${i.id}"><td><input data-k="sku" placeholder="SKU" value="${esc(i.sku)}"></td><td><input data-k="size" placeholder="Talla" value="${esc(i.size)}"></td><td><input data-k="quantity" type="number" min="1" value="${i.quantity}"></td><td><input data-k="unit_cost" type="number" min="0" step=".01" value="${i.unit_cost}"></td><td><input data-k="tax_rate" type="number" min="0" max="100" step=".001" value="${i.tax_rate}"><label class="checks"><input data-k="tax_included" type="checkbox" ${i.tax_included?'checked':''}> incluido</label><label class="checks"><input data-k="tax_creditable" type="checkbox" ${i.tax_creditable?'checked':''}> acreditable</label></td><td><input data-k="shipping_allocated" type="number" min="0" step=".01" value="${i.shipping_allocated||0}"></td><td><input data-k="duties_allocated" type="number" min="0" step=".01" value="${i.duties_allocated||0}"></td><td><input data-k="other_allocated" type="number" min="0" step=".01" value="${i.other_allocated||0}"></td><td><input data-k="margin_percent" type="number" min="0" max="1000" value="${i.margin_percent}">%</td><td>${money(c.real)}</td><td>${money(c.suggested)}</td><td><input data-k="final_price" type="number" min="0" step=".01" value="${i.final_price||c.suggested}"></td><td><button data-save>Guardar</button><button data-apply>Aplicar</button></td></tr>`}
function render(){
  $("itemsBody").innerHTML=items.map(itemRow).join("")||`<tr><td colspan="13">Selecciona una factura o agrega una partida.</td></tr>`;
...
async function load(){
  ...
  invoices=a;
  items=b.map(item=>{
    const normalized={...item,tax_included:false,tax_creditable:false};
    const linked=productById.get(Number(item.shop_product_id));
    if(!linked)return normalized;
    return {...normalized,purchase_source_sku:item.sku,sku:linked.sku||item.sku,description:linked.name||item.description,category:linked.category||item.category};
  });
