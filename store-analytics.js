(() => {
  ...
  function injectStatsButton(){const host=document.getElementById('adminTopActions');if(!host||document.getElementById('openStatisticsBtn'))return;const btn=document.createElement('button');btn.type='button';btn.id='openStatisticsBtn';btn.className='secondary-btn';btn.textContent='📊 Estadísticas';btn.addEventListener('click',async()=>{try{if(typeof requireShopAdmin==='function')await requireShopAdmin();location.href=STATS_URL}catch(err){alert(err?.message||'Inicia sesión como ADMIN.')}});host.appendChild(btn)}
