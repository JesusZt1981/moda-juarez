(() => {
  ...
  function customerClient(){
    if(sbCustomer)return sbCustomer;
    if(!window.supabase?.createClient)return null;
    sbCustomer=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false,storageKey:CUSTOMER_STORAGE_KEY}});
    return sbCustomer;
  }
  function adminClient(){
    if(sbAdmin)return sbAdmin;
    if(!window.supabase?.createClient)return null;
    sbAdmin=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false,storageKey:ADMIN_STORAGE_KEY}});
    return sbAdmin;
  }

  async function isOwnerLoggedIn(){
    for(const c of [adminClient(),customerClient()]){
      if(!c)continue;
      try{
        const {data:{session}}=await c.auth.getSession();
        if(String(session?.user?.email||'').toLowerCase()===OWNER_EMAIL)return true;
      }catch(_){}
    }
    return false;
  }
  ...
  function captureAttribution(){
    ...
    const current={
      source:q.get('utm_source')||inferSource(),
      medium:q.get('utm_medium')||'',
      campaign:q.get('utm_campaign')||'',
      content:q.get('utm_content')||'',
      term:q.get('utm_term')||'',
      campaign_id:q.get('campaign_id')||q.get('utm_campaign_id')||'',
  ...
  function sendMeta(eventName,sku,metadata={}){
    ...
    const payload={currency:'MXN'};
    if(sku)payload.content_ids=[sku];
    if(metadata?.value!=null)payload.value=Number(metadata.value)||0;
    if(sku)payload.content_type='product';
    window.fbq('track',metaName,payload,{eventID:metadata.event_id});
  }

  async function record(eventName,sku=null,metadata={}){
    if(trackingDisabled)return false;
    const c=customerClient();if(!c)return false;
    const eventId=metadata.event_id||uuid();
    const enriched={...metadata,event_id:eventId,attribution:attribution(),path:location.pathname};
    try{
      const {data,error}=await c.rpc('record_store_event',{
      ...
      sendMeta(eventName,sku,enriched);
      return data!==false;
    }catch(err){console.warn('WOMAN 656 analytics:',err?.message||err);return false}
  }
  ...
  window.W656Analytics={
    ...
    attribution,
    record,
    purchase:({sku=null,value=0,order_id=''}={})=>record('purchase',sku,{value:Number(value)||0,order_id}),
    lead:(data={})=>record('lead',null,data),
    metaEnabled:()=>Boolean(META_PIXEL_ID&&metaConsentGranted()),
